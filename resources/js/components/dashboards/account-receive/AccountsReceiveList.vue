<template>
    <div class="container-fluid">
        <div class="col-md-12">
            ...
            <search :route="'account-receive'" @search="searchParams => getAccounts(searchParams)"></search>
            <div class="card card-primary">
                <div class="card-header">
                    <h3 class="card-title"> Contas </h3>
                </div>
                <div v-if="accounts && accounts.length > 0" class="card-body table-responsive p-0" style="">
                        ...
                        <table class="table table-head-fixed text-nowrap text-left table-striped">
                            ...
                            <tbody>
                                <tr v-for="account in accounts" :key="account.id">
                                    ...
                                    <td>{{ account.final_value ? 'R$ ' + account.final_value : 'Não Informado' }}</td>
                                    <td>{{ status_account[account.status] }}</td>
                                    <td>{{ category_account_receive[account.category] }}</td>
                                    <td>{{ formatDate(account.expect_date) }}</td>
                                    <td>
                                        <router-link :to="'/account-receive/' + account.id + '/pay-off'"
                                    ...
                                    <td>
                                        <div v-if="account.status != 2">
                                            <div style="text-align: center;">
                                                <button v-if="account.deleted_at" type="button"
                                                ...
                                                <button v-else type="button" class="btn btn-outline-danger"
                                                    @click="deleteAccount(account.id)" data-toggle="modal"
                                    ...
                                    <td>
                                        <div v-if="account.status != 2 && account.category != 29">
                                            <router-link :to="'/account-receive/' + account.id + '/edit'"
                                                class="small-box-footer">
                ...
                <div v-if="totalResults >= 30" class="" style=" display: flex; justify-content: center; margin-bottom:1em; margin-top:1em;">
                    <button class="btn btn-primary" @click="goToPage(currentPage - 1)" :disabled="currentPage === 1">Anterior</button>
                    <span style="margin-left: 1em;margin-right: 1em;">Página {{ currentPage }} de {{ totalPages }}</span>
                    <button class="btn btn-primary" @click="goToPage(currentPage + 1)" :disabled="currentPage === totalPages">Próxima</button>
                </div>
...
export default {
    ...
    methods: {
        getAccounts(search = {}, page = 1) {
            ...
            search.year_received = null;
            search.month_received = null;
            axios.get('/accounts-receive', { params: search })
                .then(response => {
        ...
        delete(account) {
            axios.delete('/accounts-receive/' + account)
                .then(response => {
                    Swal.fire({
                        position: "top-end",
                        icon: "success",
        ...
        restore(account) {
            axios.post('/accounts-receive/' + account + '/restore')
                .then(response => {
                    Swal.fire({
                        position: "top-end",
                        icon: "success",
